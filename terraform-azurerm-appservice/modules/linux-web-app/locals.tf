locals {
  ...
  container_app_settings = {
    ...
    DOCKER_REGISTRY_SERVER_USERNAME     = try(var.docker_registry_username, "")
    DOCKER_REGISTRY_SERVER_URL          = try(var.docker_registry_url, "")
    DOCKER_REGISTRY_SERVER_PASSWORD     = try(var.docker_registry_password, "")
